...
excel_file_path = "/Users/nikhildhengle/Desktop/IBKR/IBJts/source/pythonclient/macd_data.xlsx"

# Create a Pandas Excel writer
with pd.ExcelWriter(excel_file_path) as writer:
    for key, values in macdDF.items():
        ...
        max_length = max(len(v) if isinstance(v, list) else 1 for v in macdDF.values())
        if len(values) < max_length:
            values.extend([None] * (max_length - len(values)))  # Fill with None for missing values

        # Create a DataFrame for the current key-value pair
        df = pd.DataFrame({key: values})

        # Write the DataFrame to a new sheet named after the key
