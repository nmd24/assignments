...
class Model_Finder:
    ...
    def get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        try:
            ...
            # create best model for NB
            self.naive_bayes=self.get_best_params_for_naive_bayes(train_x,train_y)
            self.prediction_naive_bayes=self.naive_bayes.predict(test_x) # prediction using the NB Algorithm
            ...
            if len(test_y.unique()) == 1:#if there is only one label in y, then roc_auc_score returns error. We will use accuracy in that case
                ...
                self.naive_bayes_score = roc_auc_score(test_y, self.prediction_naive_bayes) # AUC for NB
